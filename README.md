...
``template`` is optional. Specify the path and file name of IPA HFC input Excel template to be used. If ``template`` is not specified, the template will be downloaded from [IPA github page](https://github.com/PovertyAction/high-frequency-checks){:target="_blank"}, which also requires Internet connection.
...
Please report all bugs/feature request to the [github issues page](https://github.com/PovertyAction/ipachecksetup/issues){:target="_blank"}
